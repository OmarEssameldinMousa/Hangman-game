...
function FinalWord(category) {
    ...
    var spanchildrenArray = Array.from(document.querySelector(".Letter-Choices-container"))
    let alreadypressedArray = []
    ...
    document.addEventListener("click", function (e) {
        if (e.target.tagName === "SPAN") {
            if (category.includes(e.target.innerHTML)) {
                let count = 0;
                for (let i = 0; i < category.length; i++) {
                    if (category[i] === e.target.innerHTML) {
                        count++;
                ...
                // finalWordDiv.childNodes[category.indexOf(e.target.innerHTML)].innerHTML = e.target.innerHTML
    ...
    for (let i = 0; i < finalWordDiv.childNodes.length; i++) {
        if (finalWordDiv.childNodes.innerHTML.length == 0) {
            break;
        }
        else {
            window.alert("YOU HAVE WON! congrats")
        }
    }
