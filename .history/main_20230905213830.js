...
function FinalWord(category) {
    ...
    async function countOccurrences(str, char) {
        const regex = new RegExp(char, 'g');
        const matches = str.match(regex);
        return matches ? matches.length : 0;
    }

    // spanchildrenArray.childNodes.innerHTML

    for (let i = 0; i < category.length; i++) {
        charcountsArray[category[i]] = countOccurrences(category, category[i])
        charcountsArrayflag.push(0);
    }
    console.log(charcountsArray)
    console.log(charcountsArrayflag)
    ...
    document.addEventListener("click", function (e) {
        if (e.target.tagName === "SPAN") {
            if (category.includes(e.target.innerHTML) && charcountsArrayflag.indexOf(charcountsArray.indexOf(e.target.innerHTML)) < charcountsArray[e.target.innerHTML]) {
                let mycustomInex = findNthOccurrenceIndex(category, e.target.innerHTML, charcountsArrayflag[charcountsArrayflag.indexOf(charcountsArray.indexOf(e.target.innerHTML))])
                finalWordDiv.childNodes[mycustomInex].innerHTML = e.target.innerHTML
                charcountsArrayflag[charcountsArrayflag.indexOf(charcountsArray.indexOf(e.target.innerHTML))] += 1
