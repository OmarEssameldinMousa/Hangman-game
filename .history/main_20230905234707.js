...
function FinalWord(category) {
    ...
    document.addEventListener("click", function (e) {
        if (e.target.tagName === "SPAN") {
            console.log(charcountsArrayflag[charcountsArrayflag.indexOf(charcountsArray.indexOf([e.target.innerHTML,]))])
            if (category.includes(e.target.innerHTML)) {
                if (stringadded.includes(e.target.innerHTML)) {
                    let count = countOccurrences(stringadded, e.target.innerHTML)
                    let mycustomInex = findNthOccurrenceIndex(category, e.target.innerHTML, count)
                    finalWordDiv.childNodes[mycustomInex].innerHTML = e.target.innerHTML
                } else {
                    let mycustomInex = findNthOccurrenceIndex(category, e.target.innerHTML, 0)
                    finalWordDiv.childNodes[mycustomInex].innerHTML = e.target.innerHTML
                }
